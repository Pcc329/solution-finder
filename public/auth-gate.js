  ...
  function addGate() {
    ...
    gate.innerHTML = `
      <style>
        ...
        #sf-auth-gate input + label { margin-top: 16px; }
      ...
      <form class="sf-auth-card" novalidate>
        <h1>內部系統登入</h1>
        <p>請輸入帳號與密碼以使用 Solution Finder。</p>
        <label for="sf-auth-username">帳號</label>
        <input id="sf-auth-username" name="username" type="text" autocomplete="username" required autofocus>
        <label for="sf-auth-password">密碼</label>
        <input id="sf-auth-password" name="password" type="password" autocomplete="current-password" required>
    ...
    const usernameInput = gate.querySelector('#sf-auth-username');
    const passwordInput = gate.querySelector('#sf-auth-password');
